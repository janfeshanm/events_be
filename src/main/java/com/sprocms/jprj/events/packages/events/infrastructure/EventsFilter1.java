...
import com.sprocms.jprj.events.packages.events.domain.RestResponse;

import static java.util.Map.entry;

@Component
@Order(-2147483648) // 2147483648
public class EventsFilter1 implements Filter {
        ...
        public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
                ...
                responseCacheWrapperObject.setHeader("Access-Control-Allow-Origin", "*");
                responseCacheWrapperObject.setHeader("Access-Control-Allow-Methods",
                                "POST, GET, OPTIONS, DELETE, PUT, HEAD");
                responseCacheWrapperObject.setHeader("Access-Control-Max-Age", "3600");
                responseCacheWrapperObject.setHeader("Access-Control-Allow-Headers",
                ...
                if ("OPTIONS".equalsIgnoreCase(req.getMethod())) {
                        responseCacheWrapperObject.setStatus(HttpServletResponse.SC_OK);
                } else {
                        chain.doFilter(request, responseCacheWrapperObject);
                }

                HttpServletResponse res = (HttpServletResponse) responseCacheWrapperObject.getResponse();
                ...
                if (res.getStatus() != 200) {
                        ...
                        if (!(responseStr.length() > 0 && responseStr.charAt(0) == '{')) {
                                String message = RestResponse.builder()
                                                ...
                                                .error(getReasonForStatus(res.getStatus()))
                                                .message(responseStr)
                                                .path(req.getRequestURI())
                                                .json();

                                PrintWriter writer = responseCacheWrapperObject.getWriter();
